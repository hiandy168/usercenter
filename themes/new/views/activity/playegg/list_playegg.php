...
<div class="ad-act-list  bxbg ">
    ...
    <div class="ad-app-list-tit">
        ...
        <div class="fr tr">
            <a href="<?php echo $this->createUrl('/activity/playegg/add',array('pid'=>$config['pid']))?>">
    ...
    <div class="ad-nodata  mgb30 mgt30">
        ...
        <p>噢噢，还没有记录！！！</p>
        <a href="<?php echo $this->createUrl('/activity/playegg/add',array('pid'=>$config['pid']))?>" class="linear adbtn">创建活动</a>
    </div>
    <?php  }else {
    ?>
                ...
                <?php if($asList):foreach($asList as $val): ?>
                    <li class="li">
                        <div class="ad-act-list-table-con1 ">
                            <ul>
                                ...
                                <li class="lw3"><?php echo date('Y-m-d H:i:s',$val->end_time);?></li>
                                 <li class="lw2" style='cursor:pointer;'><?php $result = Activity_playegg::activityStatus($val->start_time,$val->end_time,$val->id);?>
                        ...
                        <div class="ad-act-list-table-con2">
                            <ul>
                                  <li class="l5"><a href="<?php echo $this->createUrl('/activity/playegg/prize',array('id'=>$val->id,'pid'=>$config['pid']))?>"><i></i>奖品/概率</a></li>
                                <li class="l5"><a href="<?php echo $this->createUrl('/activity/playegg/add',array('id'=>$val->id,'pid'=>$config['pid']))?>"><i></i>编辑</a></li>
                                <li class="l1"><a href="<?php echo $this->createUrl('/activity/playegg/pcview',array('id'=>$val->id))?>" target="_blank"><i></i>预览</a></li>
...
<script>
    ...
    function getWinList(param){
        ...
        layer.open({
            ...
            content: ["<?php echo $this->createUrl('/activity/playegg/WinList')?>/fid/"+param]
        });
    }
...
//活动状态提示
    function getStatus(state,fid) {
        ...
        if (state == '进行中' || state == '已结束') {
            layer.confirm('确定是否要暂停活动！抽奖类活动暂停后方可编辑', {
                ...
                var url = "<?php echo $this->createUrl('/activity/playegg/activitystatus')?>";
    ...
    function delActivity(fid){
        layer.confirm('确认要删除活动吗？', {
            ...
            $.post('<?php echo $this->createUrl('/activity/playegg/Delete')?>', { fid: fid }, function (data,status) {
                if(data.errorcode == 1){
