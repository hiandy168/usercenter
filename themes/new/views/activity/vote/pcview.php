...
<body style="background: url(<?php echo $this->_theme_url; ?>assets/images/ad-views-main-bg2.png) center; background-size: cover;">
    ...
    <div class="ad-view-main">
        ...
        <iframe  src="<?php echo "http://".$_SERVER['HTTP_HOST'].$this->createUrl('/activity/vote/view',array('id'=>$id))?>"></iframe>
        </div>
        </div>
    	
    	<div class="ad-right-views">
    		<span> <?php if($id){ ?>
                        <img src="http://qr.topscan.com/api.php?text=<?php echo $this->_siteUrl.'/activity/vote/view/id/'.$id?>" />
                        <?php }else{?>
                            <img src="http://qr.topscan.com/api.php?text=http://m.dachuw.net/h5"/>
                        <?php }?></span>
    		<p>为了您的体验更好<br />建议您扫码在手机上体验</p>
    		<div class="ad-right-views-links">
    		<textarea id="fe_text" readonly="readonly"><?php echo "http://".$_SERVER['HTTP_HOST'].$this->createUrl('/activity/vote/view',array('id'=>$id))?></textarea>
