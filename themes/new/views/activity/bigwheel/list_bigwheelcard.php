...
<?php echo $this->renderpartial('/common/assembly',array('active'=>$config['active'],'pid'=>$config['pid']))?>
    ...
    <div class="ad-app-list-tit ">
        ...
        <div class="fr tr">
            <a href="<?php echo $this->createUrl('/activity/bigwheel/add',array('pid'=>$config['pid']))?>">
    ...
    <div class="ad-nodata  mgb30 mgt30">
        ...
        <p>噢噢，还没有记录！！！</p>
        <a href="<?php echo $this->createUrl('/activity/bigwheel/add',array('pid'=>$config['pid']))?>" class="linear adbtn">创建活动</a>
    </div>
    <?php  }else{
    ?>
    ...
    function getStatus(state,fid) {
        ...
        if (state == '进行中' || state == '已结束') {
            layer.confirm('确定是否要暂停活动！抽奖类活动暂停后方可编辑', {
                ...
                var url = "<?php echo $this->createUrl('/activity/bigwheel/activitystatus')?>";
    ...
    function getActivityList(param){
        layer.confirm('查看以下哪种统计图', {
            ...
            layer.open({
                ...
                area: ['700px', '500px'],
                skin: 'layui-layer-rim', //加上边框
                content: ["<?php echo $this->createUrl('/activity/bigwheel/Activitylist')?>/fid/"+param+"/tag/pvuv"]
            });
        }, function(){
            layer.open({
                type: 2,
