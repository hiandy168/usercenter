...
<?php echo $this->renderpartial('/common/assembly',array('active'=>$config['active'],'pid'=>$config['pid']))?>
    ...
    <div class="ad-app-list-tit ">
        ...
        <div class="fr tr">
            <a href="<?php echo $this->createUrl('/activity/collectcard/add',array('pid'=>$config['pid']))?>">
    ...
    <div class="ad-nodata  mgb30 mgt30">
        ...
        <p>噢噢，还没有记录！！！</p>
        <a href="<?php echo $this->createUrl('/activity/collectcard/add',array('pid'=>$config['pid']))?>" class="linear adbtn">创建活动</a>
    </div>
    <?php  }else{
    ?>
            ...
            <ul>
                <?php if($asList):foreach($asList as $val): ?>
                    <li class="li">
                        <div class="ad-act-list-table-con1">
                            <ul>
                                ...
                                <li class="lw2" style='cursor:pointer;'><?php $result = Activity_collectcard::activityStatus($val->start_time,$val->end_time,$val->id);?>
                    ...
                    <div class="ad-act-list-wlink">
                      <h4>活动链接</h4>
                      <span><textarea name="" rows="" cols=""><?php echo $this->createUrl('/activity/collectcard/view',array('id'=>$val->id))?></textarea></span>
                      <em onclick="Copy(this)">点击复制</em>
                    </div>
                    </li> -->
                                 <li class="l5"><a href="<?php echo $this->createUrl('/activity/collectcard/prize',array('id'=>$val->id,'pid'=>$config['pid']))?>"><i></i>奖品/概率</a></li>
                                <li class="l5"><a href="<?php echo $this->createUrl('/activity/collectcard/add',array('id'=>$val->id,'pid'=>$config['pid']))?>"><i></i>编辑</a></li>
                                <li class="l1"><a href="<?php echo $this->createUrl('/activity/collectcard/pcview',array('id'=>$val->id))?>" target="_blank"><i></i>预览</a></li>
...
<script>
    ...
    function getWinList(param){
        ...
        layer.open({
            ...
            content: ["<?php echo $this->createUrl('/activity/collectcard/WinList')?>/fid/"+param]
        });
    }
    ...
    function getStatus(state,fid) {
        ...
        if (state == '进行中' || state == '已结束') {
            layer.confirm('确定是否要暂停活动！抽奖类活动暂停后方可编辑', {
                ...
                var url = "<?php echo $this->createUrl('/activity/collectcard/activitystatus')?>";
    ...
    function delActivity(fid){
        layer.confirm('确认要删除活动吗？', {
            ...
            $.post('<?php echo $this->createUrl('/activity/collectcard/Delete')?>', { fid: fid }, function (data,status) {
                if(data.errorcode == 1){
    ...
    function getActivityList(param){
        layer.confirm('查看以下哪种统计图', {
            ...
            layer.open({
                ...
                area: ['700px', '500px'],
                skin: 'layui-layer-rim', //加上边框
                content: ["<?php echo $this->createUrl('/activity/collectcard/Activitylist')?>/fid/"+param+"/tag/pvuv"]
            });
        }, function(){
            layer.open({
                type: 2,
                ...
                area: ['700px', '500px'],
                skin: 'layui-layer-rim', //加上边框
                content: ["<?php echo $this->createUrl('/activity/collectcard/Activitylist')?>/fid/"+param+"/tag/user"]
            });
        });
        //var index = layer.load(2,{shade: [0.3, '#393D49']});
