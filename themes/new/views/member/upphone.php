...
<head>
    ...
    // 修改手机号
    var mobileold=$("#mobile").val()
    $('#mobile').bind('input propertychange', function() {
       var tval=$(this).val();
       if((/^1[3578]\d{9}$/.test(tval))){
        if(tval!=mobileold){
           $("#codeshow").css('display', 'block');
        }else{
        ...
        if($("#codeshow").is(":visible")){
           ...
           if(account==''){
            ...
            $.ajax({
                ...
                url: Siteurl+'/member/upphone',  //输入验证码登录地址
                dataType: "json",
                success: function(data) {
                    if (data.state == 1) {
                ...
                <div class="login-tabcon" style="border-radius: 0px 0px 5px 5px;">
                    ...
                    <div class="login-tabcon-div">
                        <div class="login-tabcon-div1" style="display: block;">
                            <div class="login-inp has-focus"><input type="tel" name="mobile" id="mobile" placeholder="手机号" value="<?php echo $phone ?>" ></div>
                        
                        <div id="codeshow" class="login-inp has-focus" style="display: none"><input type="tel" name="smsCode" id="smsCode" value="" placeholder="验证码">
                        <a id="sendCode" href="javascript:;"><div class="login-inp-codebtn" id="send_message"><input id="hq" type="button" value="获取验证码"></div></a>
                        </div>

                        <div class="login-inp"><input type="submit" onclick="ajaxlogin()" value="确认"></div>
                        </div>
