    ...
    <?php if($activity_info['id']){ ?>
        <div class="right" style="float:right;margin:0">
            <div class="iphone" style="margin: 123px 10px 0px 12px;  height: 358px; ">
                <iframe style="width:100%;border:0px;height: 375px;" src="<?php echo $this->createUrl('/activity/pccheckin/view',array('id'=>$activity_info['id']))?>" scrolling="yes"></iframe>
            </div>
        </div>
        <div style="float: right; width: 305px;text-align: center; margin-top: 20px;">
            <a style="display: inline-block; padding: 10px 20px; border-radius: 3px; border: 1px solid #00AFD8;" target="view_window"  href="<?php echo $this->createUrl('/activity/pccheckin/view',array('id'=>$activity_info['id']))?>">请点击->>>活动链接</a>
        </div>
    <?php }else{?>
        <div class="right" style="float:right;margin:0">
            <div class="iphone" style="margin: 123px 10px 0px 12px;  height: 358px; ">
...
$('.save_button').click(function(){
    ...
    if(!title||!start_time||!desc||!end_time||!share_img){
        layer.msg("所有参数为必填");
        return false;
    }
	var data = {
