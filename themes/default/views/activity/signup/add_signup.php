...
<div style="">
            ...
            <div class="content" style="margin-top:20px;">
                ...
                <div class="t_title">活动报名图片(图片大小：640*280)</div>
                <div class="">
    ...
    <?php if($activity_info['id']){ ?>
        <div class="right" style="float:right;margin:0">
            <div class="iphone" style="margin: 123px 10px 0px 12px;  height: 358px; ">
                <iframe style="width:100%;border:0px;height: 375px;" src="<?php echo $this->createUrl('/activity/signup/view',array('id'=>$activity_info['id'],'pid'=>$config['pid']))?>" scrolling="yes"></iframe>
            </div>
        </div>
        <div style="float: right; width: 305px;text-align: center; margin-top: 20px;">
            <a style="display: inline-block; padding: 10px 20px; border-radius: 3px; border: 1px solid #00AFD8;"  target="view_window" href="<?php echo $this->createUrl('/activity/signup/view',array('id'=>$activity_info['id'],'pid'=>$config['pid']))?>">请点击->>>活动链接</a>
        </div>
    <?php }else{?>
        <div class="right" style="float:right;margin:0">
            <div class="iphone" style="margin: 123px 10px 0px 12px;  height: 358px; ">
...
var url          = "<?php echo $this->createUrl('/activity/signup/add'); ?>";
$('.save_button').click(function(){
    ...
    if(!title||!start_time||!end_time||!desc||!share_img){
        layer.msg("所有参数为必填");
        return false;
    }
	...
	$.post(url,data,function(res){
		...
		if(res.statue==1){
		    layer.msg(res.msg,{time:2000},function(){
		        window.location.href="<?php echo $this->createUrl('/activity/signup/list').'/pid/'.$config['pid'].'/active/3'; ?>";
			})
		}else{
			layer.msg(res.msg)
		}
