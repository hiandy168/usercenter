...
<div class="new_wrap">
    <div class="other_333 clearfix">
        <div class="right_222">
            <div class="i_222">组件 <span class="add-activity"><a href="<?php echo $this->createUrl('/activity/poster/add',array('pid'=>$config['pid']))?>">+ 新增活动</a><span></div>
            <div class="content_222">
                <table class="table_222">
                    ...
                    <tr>    
                        ...
                        <td><?php echo date('Y-m-d H:i:s',$val->starttime);?></td>
                        <td><?php echo date('Y-m-d H:i:s',$val->endtime);?></td>
                        <td><?php $result = Activity_scratch::activityStatus($val->starttime,$val->endtime);echo $result['message']?></td>    
                        <td>                           
		                    <a class="icon_1" href="javascript:void(0);" onclick="getWinList(<?php echo $val->id?>)"></a>
                            <a class="<?php if($result['status'] == 0 || $result['status']==-1){echo 'icon_2';}if($result['status'] == 1){echo 'icon_5';}?>" href="javascript:void(0)" onclick="getStatus('<?php echo $result['message']?>',<?php echo $val->id?>)"></a>
                            <a class="icon_3" href="<?php echo $this->createUrl('/activity/poster/add',array('fid'=>$val->id,'pid'=>$config['pid']))?>"></a>
                            <a class="icon_4" href="javascript:void(0);" onclick="delActivity(<?php echo $val->id?>)"></a>
                            <a href="<?php echo $this->createUrl('/activity/poster/view',array('id'=>$val->id))?>" target="_blank">预览</a>
                    ...
                    <tr>
                      <td colspan="6" >
                        ...
                        <textarea style="width: 74%;line-height: 26px;border: 1px solid #E6E6E6;background: #FFFFFF;"><?php echo $view['wechat_url'].urlencode($this->_siteUrl.$this->createUrl('/activity/poster/view',array('id'=>$val->id)))?></textarea>
...
<script>     
    ...
    function getWinList(param){
        ...
        layer.open({
  		  ...
  		  content: ["<?php echo $this->createUrl('/activity/poster/WinList')?>/fid/"+param]
  		});
    }
    ...
    function getStatus(state,fid){
        if(state=='未开始'){
            layer.confirm('活动还未开始？活动的开始时间将被置为当前时间！', {
              ...
              var url = "<?php echo $this->createUrl('/activity/poster/ActivityPause')?>";
    ...
    function delActivity(fid){
        layer.confirm('确认要删除活动吗？', {
          ...
          $.post('<?php echo $this->createUrl('/activity/poster/Delete')?>', { fid: fid }, function (data,status) { 
            if(data.errorcode == 1){              
