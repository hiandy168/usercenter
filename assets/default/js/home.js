...
function changeLeft(tid,obj){
	var new_url = Siteurl+'/'+$(obj).attr('ref');
        set_center(new_url);
...
function getleft_menu(type,obj){
        $.ajax({
            ...
            success: function(data, textStatus){
                ...
                //var new_url = Siteurl+$('#left_nav ul li:eq(0)').attr('ref')
                //var default_url =Siteurl+'notice';
...
function upload_file(obj,name){
	var editor = KindEditor.editor({
		fileManagerJson:Siteurl+"/files/file_manager",
		uploadJson:Siteurl+'/files/upload',
...
function upload_pic_bat(){
	var editor = KindEditor.editor({
		fileManagerJson:Siteurl+"/files/file_manager",
		uploadJson:Siteurl+'/files/upload?id='+id+'&token='+token+'&lang='+lang,
		allowFileManager : true,
                formatUploadUrl :false
	});
