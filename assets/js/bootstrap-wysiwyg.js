    ...
    //return function(jQuery){
       // var main = require('main');
        ...
        $.fn.cleanHtml = function () {
            var html = $(this).html();
            return html && html.replace(/(<br>|\s|<div><br><\/div>|<p><br><\/p>|&nbsp;)*$/, '');
        };
            ...
            var editor = this,
                ...
                bindToolbar = function (toolbar, options) {
                    ...
                    editor.prev().find('.link-dialog').on('click', function(){
                        main.form_dialog({
                            content: '<div class="row-fluid control-group" id="add_url_dialog"><label class="control-label" for="menu_title">请输入合法的URL(http://开头)：</label><input class="span12" placeholder="http://" type="text" value="http://" id="add_url"></div>',
                            form_rules: function(){
                                var reg_text = /^http[s]?:\/\/.*/;
                            ...
                            callback: function(obj){
                                ...
                                if (newValue) {
                                    ...
                                    if(selectedRange.toString() === ''){
                                        ...
                                        if(selectedRange.endOffset === 0){
                                            var nnode = document.createElement('a');
                                            nnode.href = newValue;
                                            selectedRange.surroundContents(nnode);
                    ...
                    editor.prev().find('.phone-dialog').on('click', function(){
                        main.form_dialog({
                            content: '<div class="row-fluid control-group" id="add_phone_dialog"><label class="control-label" for="menu_title">请输入您的电话号码（只能为数字）：</label><input class="span12" placeholder="02988888888" type="text" value="" id="add_phone"></div>',
                            form_rules: function(){
                                var reg_text = /^([+-]?)\d*\.?\d+$/;
                    ...
                    //变换字体颜色
                    editor.prev().find('.select_font_color span').on('click', function(){
...
//                            var color = $(this).find('a').css('background-color');
                    ...
                    //设置背景色
                    editor.prev().find('.select_bg_color span').on('click', function(){
                        if($(this).hasClass('color_none')){
