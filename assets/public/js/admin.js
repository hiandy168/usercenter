$(document).ready(function() {
    ...
    $('#verify_image').click(
            function() {
                $('#verify_image').attr('src', admin_url + '/login/verify_image?' + Math.random());
...
function SetHome(obj, vrl) {
    try {
        ...
        obj.setHomePage(vrl = 'http://www.9open.net');
    }
    catch (e) {
        if (window.netscape) {
            try {
...
$(".tabs li").click(function() {
    var thisparent = $(this).parent('ul').parents('.tabs');
...
function  dofocus(url, id, status) {
    if (status) {
        var content = '确定设置为聚焦?';
    } else {
        var content = '确定取消聚焦?';
...
function  dochoiceness(url, id, status) {
    if (status) {
        var content = '确定设置为精选?';
    } else {
        var content = '确定取消精选?';
