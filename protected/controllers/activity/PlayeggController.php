 ...
 *
 */
class PlayeggController extends FrontController {
    public function init() {
       ...
       if(!$this->member && !$_SESSION['mid']){
        ...
        $this->activity_permissions('playegg');
    }
    ...
    public function actionView() {
        ...
        //查询砸金蛋信息
        $sql = "SELECT * FROM {{activity_playegg}} WHERE id=$id";
        ...
        Browse::add_activity_browse($info['pid'],$id,"playegg");
...
//        $backUrl = "?id=".$id."&accesstoken=".$token."&openid=".$openid;
        $all_prize_count = 0 ;//砸金蛋总奖品数量
        $prize_id = explode(',',rtrim($info['prize_id'],','));
        foreach ($prize_id as $key=>$val){
            //查询奖品信息
            $sql = "SELECT * FROM {{activity_playegg_prize}} WHERE id=$val and status =1";
...
//                "status" => $mid,
            ...
            'config'=>array(
                'site_title'=> $info['title'].'-砸金蛋抽奖',
                'Keywords'=>$info['title'].',砸金蛋,抽奖,一等奖',
                'Description'=>$info['title'].',砸金蛋,抽奖,一等奖',
            ),
            'all_prize_count'=>$all_prize_count
        );
        $this->render('view_playegg',$parame);
    }
    ...
    public function actionis_status(){
        ...
        if (Mod::app()->request->isPostRequest) {
            ...
            if($id){
                $res=  Activity_playegg::model()->findByPk($id);
                if($res){
    ...
    public function actionlist(){
        ...
        //end权限
         ...
         //砸金蛋活动列表 
         $as_list = Activity_playegg::model()->getActivityListPager($pid);
//        if(1==1) { //default模板风格
//            if (!$as_list['count']) {
//                $redirect_url = Mod::app()->baseUrl . '/activity/palyegg/add/pid/' . $pid;
//                $this->redirect($redirect_url);
//            }
//        }
        $config['site_title'] = '砸金蛋活动列表-大楚网用户开放平台';
        $config['site_keywords'] = "大楚网用户开放平台,砸金蛋，活动";
        $config['site_description'] ="大楚网用户开放平台_砸金蛋活动页面";
         $config['active_1']=3;
        $config['active'] =9;
         $config['pid']=$pid;
         $parame = array(
         ...
         $this->render('list_playegg',$parame);
    }
   ...
   * */
    ...
    public function actionPrize(){
        ...
        if($activity_id){//编辑
                //判断是不是自己的所属项目 不是没有权限
                $sql = "select * from {{activity_playegg}} where id=$activity_id";
        ...
        //end所属权限   
          ...
          if (Mod::app()->request->isPostRequest) {
                   ...
                   if($activity_info['status']==1){
                    ...
                    //步骤1：查找之前的奖品
                    $prize_arr_id = array();
                    $sql = "select * from {{activity_playegg_prize}} where aid=".$activity_info['id']." and status =1";
         ...
         //                    步骤2：对比现在的奖品  更新之前编辑的奖品
                            ...
                            foreach ($prize_id_intersect as $key => $val) {
                                ...
                                if(in_array($val,$prize_arr_id)){
                                    ...
                                    //查询历史中数量
                                     $sql = "select * from {{activity_playegg_user}} where playegg_id =".$activity_info['id']." and prize_id = ".$val." and is_win =1 ";
...
//
                                    ...
                                    $res = Mod::app()->db->createCommand()->update('{{activity_playegg_prize}}', $prize_data, 'id=:id', $update_id);
        ...
        //                   步骤3：对比现在的奖品  有删除的就删除掉    
                            foreach ($prize_id_diff as $key => $val) {
                                //删除的 
                                if(in_array($val,$prize_arr_id)){
                                    Mod::app()->db->createCommand()->update('{{activity_playegg_prize}}', array('aid'=>$activity_info['id'],'status'=>0,'updatetime'=>time()), 'id='.$val);
                                } 
                            }
        ...
        //                    步骤5：对比现在的奖品  写入新增的奖品        
                            foreach($p_id as $key => $val ){
                                ...
                                if(!$val){
                                    ...
                                    $prize_data['aid']=$activity_info['id'];  
                                    $prize_data['status'] = 1;
                                    $prize_model = new Activity_playegg_prize();
                            ...
                            Activity_playegg::model()->updateByPk($activity_id, array('prize_id'=> implode(',',$new_prize_arr_id),'jishu'=>Tool::getValidParam('jishu')));//更新基数 ，更新奖品字段冗余吧 之前开发写的 我也更新下吧
                   ...
                   $activity_playegg_info = $activity_info;
                    ...
                    $config['active_1'] = '3';
                    //组件assembly中的选中高亮背景图片 砸金蛋(1)、签到(2)、报名(3)
                    $config['active'] = 9;
                    $config['site_title']='奖品设置-编辑砸金蛋活动-大楚网用户开放平台';
                    $config['Keywords']='大楚网用户开放平台,砸金蛋，抽奖，一等奖';
                    $config['Description']='添加砸金蛋活动_编辑砸金蛋活动'; 
 
                    $parame = array(
                        'config' => $config,
                        'activity_info' =>$activity_playegg_info,
                        'prize' => $prize,
                        'status' => $this->activity_status('playegg'),
                    );
      
                    $this->render('prize_playegg', $parame);
        }
    ...
    public function actionAdd(){
        if(!$this->member  ||  !$this->member['id'] || !$this->member['pstatus']){
         ...
         if(Mod::app()->request->isPostRequest){
             ...
             $sql = "SHOW FULL FIELDS FROM {{activity_playegg}}";
            ...
            if($activity_id){
                ...
                $query = Mod::app()->db->createCommand()->update('{{activity_playegg}}',$arr,'id=:id', $update_id);
                $str ='编辑';
            }else{
                $arr['add_time']  =time();
                $query = Mod::app()->db->createCommand()->insert('{{activity_playegg}}',$arr);
                $str ='添加';
                if($query){
                    //新增加活动之后发送消息
                    ...
                    //更新奖品的aid字段
                    if($arr['prize_id'] && $id){
                        Activity_playegg_prize::model()->updateAll(array('aid' => $id), 'id  in ('.$arr['prize_id'].')');
                    }
                    
                    $tablename="playegg";
                    $url=$this->_siteUrl."/activity/playegg/view/id/".$id;
            ...
            //获取点击编辑是得到的活动id
             ...
             if ($fid) {
                 //start所属权限开始
                 $sql = "select * from {{activity_playegg}} where id=$fid";
                 $activity_info = Mod::app()->db->createCommand($sql)->queryRow();
                 if(!$activity_info['pid']){die('数据非法');}
                 //防止ID遍历
                 $projectinfo =  JkCms::getprojectByid($activity_info['pid']);
             ...
             //head_app中的 应用首页（1）、基础配置（2）、应用组件（3）三个按钮选中加背景
             $config['active_1'] ='3';
             //组件assembly中的选中高亮背景图片
             $config['active']=9;
             $config['site_title'] = '添加编辑砸金蛋活动组件-砸金蛋活动组件-大楚网用户开放平台';
             $config['site_keywords'] = "大楚网用户开放平台,砸金蛋，活动";
             $config['site_description'] ="大楚网用户开放平台_砸金蛋活动页面";
             $config['pid']=$pid;
             ...
             $this->render('add_playegg',$parame);
        }
    }
    ...
    public function actionDelete(){
        ...
        if(Mod::app()->request->isAjaxRequest){
            ...
            if($fid){
                //防止ID遍历
                $respcck=Activity_playegg::model()->findByPk($fid);
                if(!$respcck){
                    $result = array(
                        'errorcode'=>0
                    );
                ...
                //查询砸金蛋中的奖品
                $playegg = Mod::app()->db->createCommand()->select('*')->from('{{activity_playegg}}')->where('id='.$fid)->queryRow();
                $prizeid = rtrim($playegg['prize_id'],',');
                $arr_id = explode(',',$prizeid);
                foreach($arr_id as $val){
                    Mod::app()->db->createCommand()->delete('{{activity_playegg_prize}}', 'id IN('.$prizeid.')');
                }
                $res = Mod::app()->db->createCommand()->delete('{{activity_playegg}}', 'id IN('.$fid.')');
                if($res){
    ...
    public function actionUserWinPrize(){
        ...
        $win_prize_res = Mod::app()->db->createCommand()->select('*')->from('{{activity_playegg_user}}')->where("playegg_id=:playegg_id and mid=:mid  and is_win=:is_win",$where)->queryAll();
        if($win_prize_res){
            foreach ($win_prize_res as $key=>$val){
                $id = $val['prize_id'];
                $sql = "SELECT * FROM {{activity_playegg_prize}} WHERE id=$id and status =1";
    ...
    public function actionGetWin(){
        ...
        // foreach ($prize_id as $key=>$val){
        //     //查询奖品信息
        //     $sql = "SELECT * FROM {{activity_playegg_prize}} WHERE id=$val";
        //     $prize[$key]=Mod::app()->db->createCommand($sql)->queryRow();
        // }

        $prize_id = rtrim($info['prize_id'],',');
        $sql = "SELECT * FROM {{activity_playegg_prize}} WHERE status=1 and id IN ($prize_id)";
        $prize=Mod::app()->db->createCommand($sql)->queryAll();
        if(!$prize){die('数据不合法');}
        ...
        $sql="select count(id) as con  from {{activity_playegg_user}} where playegg_id=".$id." and  mid=".$mid." and is_win=1";
        ...
        if($zj_con['con'] >= $info['win_num']){
            ...
            $sql = "SELECT * FROM {{activity_playegg_prize}} WHERE id=$prize_id  and status =1";
            $prize_info = Mod::app()->db->createCommand($sql)->queryRow();
            if($prize_info['remainder']>0){
                 $prize_id = $prize_id;
                //查询中奖的id 该用户是否中过此等奖 防止重复中奖
                $re=  Activity_playegg_user::model()->find('prize_id=:prize_id AND playegg_id=:playegg_id AND mid=:mid',array(':prize_id'=>$prize_id,':playegg_id'=>$id,':mid'=>$this->member['id']));
                if($re){
                    $prize_id = 0;
                }
            }else{
        ...
        //得到该用户在当天参加的次数
        $usercount = Activity_playegg_user::model()->count('mid=:mid AND time>:time AND time<:timeend AND playegg_id=:playegg_id', array(':mid' => $mid, ':time' => $wheretime, ':timeend' => $endtime, ':playegg_id' => $id));
...
try {
        if ($prize_id && $dayCount) {//表中奖
            ...
            //根据奖品id查询奖品信息
            $sql = "SELECT * FROM {{activity_playegg_prize}} WHERE id=$prize_id  and status=1 for update";
            $prize_info=Mod::app()->db->createCommand($sql)->queryRow();
            $data_user['is_win']  = 1;
            ...
            //如果中奖则减少奖品
                ...
                $sql = " UPDATE {{activity_playegg_prize}} SET remainder = remainder-1 WHERE id = $prize_id  and status=1";
    ...
    $transaction->rollBack();
        ...
        $res = Behavior::behavior_points(5,$mid,$info['pid'],$name,$win,$id,'activity_playegg');
    ...
    public function actionWinlist(){
      ...
      /*  if(!empty($username)){
        ...
        $as_list = Activity_playegg_user::model()->getUserListPager($fid,$is_win,$search,$username);
        if($as_list['count']){
            foreach($as_list['criteria'] as $key=>$val){
                ...
                if($prizeid){
                    $sql = "select * from {{activity_playegg_prize}} where id = $prizeid";
                    $prize = Mod::app()->db->createCommand($sql)->queryRow();
                }else{
                    $prize = array();
                }
                $as_list['users'][$key]['id']=$val['id'];
                $as_list['users'][$key]['playegg_id']=$val['playegg_id'];
    ...
    public function actionLingjiang(){
        $id     = trim(Tool::getValidParam('id','integer'));
        //start所属权限开始
        $activity_id=Activity_playegg_user::model()->findByPk($id);

       
        if($activity_id){//编辑

            //判断是不是自己的所属项目 不是没有权限
            $sql = "select * from {{activity_playegg}} where id=$activity_id->playegg_id";
            $activity_info = Mod::app()->db->createCommand($sql)->queryRow();
            if(!$activity_info['pid']){die('数据非法');}
            //防止ID遍历
            $projectinfo =  JkCms::getprojectByid($activity_info['pid']);
        ...
        $res = Mod::app()->db->createCommand()->update('{{activity_playegg_user}}',$update_data,'id=:id',$where_data);
        if($res){
            $arr = array(
                'code'=>1,
                'msg'=>'设置领奖成功'
    ...
    public function actionExportCsv(){
        ...
        //end权限
        if($datatype == 1){
            $where = "playegg_id = $fid and is_win = 1";
        }elseif($datatype==2){
            $where = "playegg_id = $fid and is_win = 0";
        }else{
            $where = "playegg_id = $fid";
        }
        Mod::import('ext.ECSVExport');
        $list = Mod::app()->db->createCommand()  
           ->select('*')  
           ->from('{{activity_playegg_user}}')
           ->where($where)
           ->queryAll();  
        if($list){
            foreach($list as $key=>$val){
                ...
                $as_list[$key]['playegg_id']=$val['playegg_id'];
    ...
    public function actionPcview()
    {
        ...
        $parame = array(
            'id' => $id,
            'config'=>array(
                'site_title'=> '砸金蛋活动页面-pc版活动页面-大楚网用户开放平台',
                'Keywords'=>'砸金蛋活动页面-pc版活动页面-大楚网用户开放平台',
                'Description'=>'砸金蛋活动页面-pc版活动页面-大楚网用户开放平台'
            ),
        );
    ...
    public function actionActivitylist(){
        ...
        switch ($config['tag']) {
            ...
            case "user":
                ...
                $table_user = "dym_activity_".playegg."_user";
                $data['signup'] = Mod::app()->db->createCommand()->select('count(0)')->from('dym_member_activity')->where('aid=' . $config['aid'] . ' and model = "' . playegg . '" and (createtime between '.strtotime($last).' and '.$now.')')->queryRow();
                $data['join'] = Mod::app()->db->createCommand()->select('count(0)')->from($table_user)->where('playegg_id=' . $config['aid'] . '  and (time between '.strtotime($last).' and '.$now.')')->queryRow();
