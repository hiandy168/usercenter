...
//        }
    ...
    public function actionPrize()
    {
        ...
        if ($activity_id) {//编辑
            ...
            if ($this->memberverify($projectinfo['mid'])) {
                echo json_encode(array('state' => 0, 'msg' => '非法访问'));
                exit;
            }
        } else {
            die('非法访问');
                ...
                foreach ($prize_id_intersect as $key => $val) {
                    ...
                    if (in_array($val, $prize_arr_id)) {
                        ...
                        if (($prize_data['remainder'] + count($this_win_list)) > $prize_data['count']) {
                            ...
                            //严重数据错误
                            echo json_encode(array('state' => 0, 'msg' => '中奖的数量加上+剩余的数量已经大于奖品的数量'));
                            exit;
//                                           die('中奖的数量加上+剩余的数量已经大于奖品的数量');
                        ...
                        if (($prize_data['remainder'] + count($this_win_list)) != $prize_data['count']) {
                            ...
                            //严重数据错误
                            echo json_encode(array('state' => 0, 'msg' => '中奖的数量加上+剩余的数量不等于奖品的数量，现在已中奖数为' . +count($this_win_list)));
                            exit;
                        }
                ...
                if (count($new_prize_arr_id) < 4 || count($new_prize_arr_id) > 12) {
                    $transaction->rollBack();
                    echo json_encode(array('state' => 0, 'msg' => '集卡的奖品种类至少是4张,最多12张'));
                    exit;
                }
                ...
                $transaction->commit();
                echo json_encode(array('state' => 1, 'aid' => $activity_id, 'msg' => '修改成功'));
                exit;
    ...
    public function actionUploadimg()
    {
        ...
        if (Mod::app()->request->isPostRequest) {
            ...
            if ($imgre) {
                echo json_encode(array('state' => 1, 'msg' => $str . '成功'));
                exit;
            } else {
                //如果保存失败，删除已经给上产的文件
                foreach ($img as $v) {
                ...
                echo json_encode(array('state' => 0, 'msg' => $str . '失败， 请联系管理员！'));
                exit;
            }
    ...
    public function actionGetWin()
    {
        ...
        $where = array(
            'group' => 'prize_id',
            'condition' => 'mid=:mid and collectcard_id=:cid',
            'params' => array(":mid" => $mid, ":cid" => $id),
        );
        $yes = Activity_collectcard_user::model()->count($where);
        $pnum = Activity_collectcard_prize::model()->count("aid=:aid", array(":aid" => $id));
        if ($yes >= $pnum) {//如果该用户收集的卡片单类大于等于卡片单类 说明肯定集齐了
            //如果集齐卡片就在user表标记一条数据 is_win =99
            $res = Activity_collectcard_user::model()->find('mid=:mid and collectcard_id=:cid and is_win=:is_win', array(":mid" => $mid, ":cid" => $id, ":is_win" => 99));
            if (!$res) {
                $arr['is_win'] = 99;
                $arr['time'] = time();
                $update_id = array(':mid' => $mid);
                $re = Mod::app()->db->createCommand()->update('{{activity_collectcard_user}}', $arr, 'mid=:mid', $update_id);
                if ($re) {
                    //集齐之后再收集不到卡片了
                    $prize_id = 0;
                } else {
...
//                    }
    ...
    public function actionWinlist()
    {
        ...
        if ($as_list['count']) {
            foreach ($as_list['criteria'] as $key => $val) {
                ...
                $as_list['users'][$key]['is_win'] = $val['is_win'];
                ...
                //未集齐的用户
                if($is_win==2 ){
                    $res = Activity_collectcard_user::model()->find('mid=:mid and collectcard_id=:cid and is_win=:is_win', array(":mid" => $mid, ":cid" => $activity_id, ":is_win" => 99));
                    if($res){
                        unset($as_list['users'][$key]);
                    }
                }
        ...
        $is_win= $username?1:$is_win;
        $as_list['active'] = $active;
        $as_list['id'] = $fid;
        $as_list['activity_info'] = $activity_info;
        $as_list['is_win'] = $is_win;
    ...
    public function actionExportCsv()
    {
        ...
        if ($datatype == 1) {
            $where = "collectcard_id = $fid and is_win = 99";
        } elseif ($datatype == 2) {
        ...
        if ($as_list) {
            foreach ($as_list as $k => $v) {
                ...
                $list[$k]['集齐时间'] = date('Y-m-d H:i:s', $v['time']);
                $list[$k]['领奖状态'] = $v['accept'] == 1 ? '已经领奖' : '没有领奖';
                $list[$k]['是否集齐'] = $v['is_win'] == 99 ? '是' : '否';
            }
        }
        $data = array();
        foreach ($list as $key => $val) {
