 ...
 * 积分管理系统的控制器
 */
class ActivityController extends HaController
{

    //活动管理列表
    public function actionList()
    {
        $id = intval($_GET['id']);//h活动组件id， //    2刮刮卡  1签到  3报名
        $project = intval($_GET['project']);//项目id
        ...
        switch ($id) {
            case 1:
                $res = Mod::app()->db->createCommand()->select('*')->from('dym_activity_pccheckin')->where('pid='.$projectid)->queryAll();
                break;
            case 3:
                $res = Mod::app()->db->createCommand()->select('*')->from('dym_activity_signup')->where('pid='.$projectid)->queryAll();
                break;
            case 4:
                $res = Mod::app()->db->createCommand()->select('*')->from('dym_activity_vote')->where('pid='.$projectid)->queryAll();
                break;
            case 5:
                $res = Mod::app()->db->createCommand()->select('*')->from('dym_activity_bigwheel')->where('pid='.$projectid)->queryAll();
                break;
            case 6:
                $res = Mod::app()->db->createCommand()->select('*')->from('dym_activity_poster')->where('pid='.$projectid)->queryAll();
                break;
            default:
                $res = Mod::app()->db->createCommand()->select('id,pid,prize_id,title,start_time,end_time,win_num,day_count,share_num,share_add_num,win_msg,rule,lingjiang,end_num_msg,end_msg,jishu,share_img as img,banner_img,bg_img,scratch_img,desc_img,statue,add_time')->from('dym_activity_scratch')->where('pid='.$projectid)->queryAll();
                break;
        }
        ...
        $this->render('list', array('datalist' => $res,'id'=>$id, 'type_id' => $type[$id],'project'=>$project,'projectid'=>$projectid));
    }


    public function actionSignupEdit(){
        echo 123;
    }
    ...
    public function actionRecommendedList(){
        ...
        $list = Mod::app()->db->createCommand()->select('*')->from('dym_activity_recommend')->where('status=1')->queryAll();
