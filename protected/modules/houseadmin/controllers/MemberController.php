...
class MemberController extends HaController {

    public function actionIndex(){
        $status= Tool::getValidParam('status','integer');
           $this->actionlists($status);
    }
        
    public function actionlists ($status)
    {
        ...
        $criteria->addCondition("t.group_id=".$group_id);
         ...
         if(isset($_REQUEST['name'])&& $_REQUEST['name']){
           $criteria->addCondition("t.name like '%".trim($_REQUEST['name']."%'"));
            $data['s']['name'] = trim($_REQUEST['name']);
        }
        ...
        $this->render('member', array ('s'=>$data['s'],'datalist' => $result ,'group'=>$group, 'pagebar' => $pages ));
    }
...
//            file_put_contents('/text.txt', var_export($id_arr, 1));
    ...
    public function actionpclists ()
    {
        ...
        $criteria->offset = $pages->currentPage * $pages->pageSize;
        $result = $model->findAll($criteria);
        $this->render('pcmember', array ('datalist' => $result ,'group'=>$group, 'pagebar' => $pages ));
    }
