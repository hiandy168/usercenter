...
//        $result = Article::model()->findAll();
        ...
        if(isset($_POST['type_id'])&& $_POST['type_id']){
           $criteria->addCondition("t.id=".intval($_POST['type_id']));
        ...
        $this->check_permission(__CLASS__, __FUNCTION__);
          if( !empty($_POST) ){
	        ...
	        foreach($model->attributes as $k=>$v){
                ...
                if($model->save()){
                    $target_url = $this->createUrl('/admin/type/');
            ...
            //获取幻灯片
             $typemodel =Models::model()->findAll();
             foreach($typemodel as $cm){
                 $data['typearr'][] = $cm->attributes;
             }
