...
class AdsController extends AController {
    ...
    public function actionPreview() {
       ...
       if($type){
            ...
            foreach($ads as $a){
                if($a['type']==1){
                    echo   '<a href="'.$a['url'].'">';
                    echo   '<img src="'.JkCms::show_img($a['picture']).'" style="'.(intval($a['width'])?'width:'.intval($a['width']).'px;':'').(intval($a['height'])?'height:'.intval($a['height']).'px;':'').'">';
                    echo   '</a>';
                }else if($a['type']==2){           
                   echo $a['code'];
                }
