...
//        }
        ...
        //获取文章栏目
        $categorymodel =  Category::model()->findAll("lang=:lang and model = 'ask'",array(":lang"=>$this->lang));
        foreach($categorymodel as $c){
                $data['categoryarr'][$c->id] = $c->attributes;
        }
        
        if(isset($data['categoryarr'])){
...
//                 $model->picture = ltrim($data['picture'], $this->_siteUrl.'/');
                   $model->createtime = $model->updatetime = strtotime($data['createtime']);
...
//                $model->lang = $this->lang;
                if($model->save()){
                    $target_url = $this->createUrl('/admin/ask/lists');
            ...
            //获取栏目
              $categorymodel =Category::model()->findAll("lang=:lang and model=:model",array(":lang"=>$this->lang,':model'=>'ask'));
