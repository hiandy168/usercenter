...
<div class="center_top clearfix">
    ...
    <div class="center_search"> 
        <form name="search_frm" action="<?php echo $this->createUrl("pic/lists")?>" id="SearchFrm" method="post"> 
                  <select name="type_id">
    ...
    <?php } ?>
    <a  style="margin:3px 20px;" class="btn btn-primary" href="<?php echo $this->createUrl('pic/add')?>">添加</a>
</div>
  ...
  <table width="100%" cellspacing="0">
		<thead>
			<tr>
			  ...
			  <th width="150"><?php echo Mod::t('admin','operation')?></th>
			</tr>
		</thead>
		<tbody>	
                     <?php if(!empty($list)){foreach($list as $k=>$item){?>
                        <tr id="list_<?php echo $item['id']?>">
                          ...
                          <td>
                              <a class="a_pic" style='width:35px;' href="<?php echo $this->createUrl("pic/pictures/",array('id'=>$item['id']))?>"><?php echo Mod::t('admin','pic')?></a>  
                              <a class="a_edit" href="<?php echo $this->createUrl("pic/edit/",array('id'=>$item['id']))?>">编辑</a> 
                              <a class="a_del" onclick="del('<?php echo $this->createUrl("pic/del")?>','<?php echo $item['id'] ?>')" href="javascript:;">删除</a>
                          </td>
                        </tr>	
                    <?php }}else{ ?>
...
<div class="center_footer clearfix">
        <ul>
            ...
            <li><a  class="btn btn-primary" href="javascript:;" onclick="del_bat('<?php echo $this->createUrl("pic/del")?>')">删除</a></li>      
            <li><a  class="btn btn-primary" href="javascript:;" onclick="submitorder('<?php echo $this->createUrl("pic/order")?>')">排序</a></li>   
