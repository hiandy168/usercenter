...
<div class="center_top clearfix">
    ...
    <div class="center_search"> 
        <form name="search_frm" action="<?php echo $this->createUrl("product/lists")?>" id="SearchFrm" method="post"> 
                ...
                <label>栏目：</label> <select name="category_id">
                    ...
                    <option value="<?php echo $category['id']?>" <?php if (isset($s['category_id'])&&$s['category_id']==$category['id']): ?>selected<?php endif; ?>><?php echo $category['name']?></option>
                    <?php endforeach;?>
		</select>
                <label>推荐位：</label>
    ...
    <?php } ?>
    <a  style="float:right;" class="btn btn-primary" href="<?php echo $this->createUrl('product/add')?>">添加</a>
</div>
		...
		<tbody>	
                     <?php if(!empty($list)){foreach($list as $k=>$item){?>
                        <tr id="list_<?php echo $item['id']?>">
                          ...
                          <td><a class="a_edit" href="<?php echo $this->createUrl("product/edit/",array('id'=>$item['id']))?>">编辑</a> <a class="a_del" onclick="del('<?php echo $this->createUrl("product/del")?>','<?php echo $item['id'] ?>')" href="javascript:;">删除</a></td>
                        </tr>	
                     <?php }}else{ ?>
...
<div class="center_footer clearfix">
        <ul>
            ...
            <li><a class="btn btn-primary" href="javascript:;" onclick="del_bat('<?php echo $this->createUrl("product/del")?>')">删除</a></li>    
            <li style='margin:0 0 0 5px;'><a class="btn btn-primary" href="javascript:;" onclick="submitorder('<?php echo $this->createUrl("product/order")?>')">排序</a></li>   
