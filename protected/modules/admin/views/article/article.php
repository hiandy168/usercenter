...
<div class="center_top clearfix">
    ...
    <div class="center_search"> 
        <form name="search_frm" action="<?php echo $this->createUrl("article/lists")?>" id="SearchFrm" method="post"> 
    ...
    <?php } ?>
    <a  style="float:right;" class="btn btn-primary" href="<?php echo $this->createUrl('article/add',array('category_id'=>$s['category_id']))?>">添加</a>
</div>
		...
		<tbody>	
                     <?php if(!empty($list)){foreach($list as $k=>$item){?>
                        <tr id="list_<?php echo $item['id']?>">
                          ...
                          <td><span style='color:#06c'>[<?php echo  $categoryarr[$item['category_id']]['name'] ?>]</span><?php echo $item['title']?><?php if($item['top']){?><span style='color:#ff0000'>顶置</span><?php } ?></td>
                          <td><?php echo $item['hits']?></td>
                          <td><?php echo $item['status']?'启用':'禁用'; ?></td>
                          <td><?php echo date('Y-m-d H:i:s',$item['createtime'])?></td>
                          <td><a class="a_edit" href="<?php echo $this->createUrl("article/edit/",array('id'=>$item['id']))?>">编辑</a> <a class="a_del" onclick="del('<?php echo $this->createUrl("article/del")?>','<?php echo $item['id'] ?>')" href="javascript:;">删除</a></td>
                        </tr>	
                     <?php }}else{ ?>
...
<div class="center_footer clearfix">
        <ul>
            ...
            <li><a class="btn btn-primary" href="javascript:;" onclick="del_bat('<?php echo $this->createUrl("article/del")?>')">删除</a></li>    
            <li style='margin:0 0 0 5px;'><a class="btn btn-primary" href="javascript:;" onclick="submitorder('<?php echo $this->createUrl("article/order")?>')">排序</a></li>   
