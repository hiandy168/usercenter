...
<div class="center_top">
    ...
    <div class="control_nav"> 
        <ul>
            <li><span  class="btn btn-primary">友情链接管理</span></li>
            <li><a class="btn btn-primary" href="<?php echo $this->createUrl('friendlink/add')?>">添加</a></li>   
        </ul>
    </div>
    <?php if(!empty($type_arr)){?>
    <div class="center_search"> 
        <form name="search_frm" action="<?php echo $this->createUrl("/admin/friendlink/lists")?>" id="SearchFrm" method="post"> 
                  <select name="type_id">
...
<div class="list">
		...
		<tbody>	
                     <?php if(!empty($list)){foreach($list as $k=>$item){?>
                        <tr id="list_<?php echo $item['id']?>">
                          ...
                          <td><?php if($item['picture']){ ?><img height="50" src="<?php echo JkCms::show_img($item['picture']) ?>"><?php } ?></td>
                          <td><a href="<?php echo $item['url']?>" target="_blank"><?php echo $item['url']?></a></td>
                          <td>
                              <a class='a_edit' href="<?php echo $this->createUrl("friendlink/edit/",array('id'=>$item['id']))?>">编辑</a> 
                              <a class='a_del'  onclick="del('<?php echo $this->createUrl("friendlink/del")?>','<?php echo $item['id'] ?>')" href="javascript:;">删除</a>
                          </td>
                        </tr>	
                     <?php }} ?>
		</tbody>
  ...
  <li><a class="btn btn-primary" href="javascript:;" onclick="del_bat('<?php echo $this->createUrl("friendlink/del")?>')">删除</a></li>    
            <li><a class="btn btn-primary" href="javascript:;" onclick="submitorder('<?php echo $this->createUrl("friendlink/order")?>')">排序</a></li>   
        </ul>
 </div>  
</div>
 </div>   
