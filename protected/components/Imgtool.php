...
//*/
  ...
  * @return boolean 
  */
  public function create_thumb($source, $dest,$width='',$height='',$type='fit'){ 
    ...
    $this->_height = $height?$height:$this->height;
  ...
  * @return boolean 
  */
  public function create_thumb_fit($source, $dest,$width='',$height=''){ 
  ...
  * @return boolean 
  */
  public function create_thumb_crop($source, $dest,$width='',$height=''){ 
...
//    list($pic_w, $pic_h) = $this->get_size(); 
    // 获取截图的偏移量 
    list($offset_w, $offset_h) = $this->get_crop_offset(); 
    // 判断是否要转为RGB 
    $source_info = getimagesize($this->_source); 
    $colorspace = (!isset($source_info['channels']) || $source_info['channels']!=3)? ' -colorspace RGB ' : ''; 
...
//    $cmd = sprintf("convert -resize '%sx%s' '%s' -quality %s %s -crop %sx%s+%s+%s +repage '%s'", $pic_w, $pic_h, $this->_source, $this->_quality, $colorspace, $this->_width, $this->_height, $offset_w, $offset_h, $this->_dest); 
      $cmd = sprintf("convert %s -quality %s %s -crop %sx%s+%s+%s +repage %s",  $this->_source, $this->_quality, $colorspace, $this->_width, $this->_height, $offset_w, $offset_h, $this->_dest); 
  ...
  */
  private function gd_crop(){ 
    ...
    imagecopyresampled($new_img, $source_img, 0, 0, $offset_w, $offset_h,$this->_width,$this->_height,$this->_width,$this->_height); 
    ...
    switch($dest_ext){ 
      ...
      case 2: imagejpeg($new_img, $this->_dest, $this->_quality); break; 
      case 3: imagepng($new_img, $this->_dest, (int)(($this->_quality-1)/10)); break; 
//      case 6: imagebmp($new_img, $this->_dest, (int)(($this->_quality-1)/10)); break; 
    } 
  ...
  */
  private function get_crop_offset(){ 
