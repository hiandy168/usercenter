...
class Wechat
{
	...
	{
		//Tool::getValidParam("signature");  这个方法是新的，作用是在不知道参数类型的情况下，自动识别!
		$signature=Tool::getValidParam("signature");
		$msg_signature=Tool::getValidParam("msg_signature");
		$timestamp=Tool::getValidParam("timestamp");
		$nonce=Tool::getValidParam("nonce");

        $signature = isset($signature)?$signature:'';
	    $signature = isset($msg_signature)?$msg_signature:$signature; //如果存在加密验证则用加密验证段
	...
	public function valid($return=false)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
			$encrypt_type=Tool::getValidParam("encrypt_type");
            $postStr = file_get_contents("php://input");
            $array = (array)simplexml_load_string($postStr, 'SimpleXMLElement', LIBXML_NOCDATA);
            $this->encrypt_type = isset($encrypt_type) ? $encrypt_type: '';
        	...
        	$echoStr = Tool::getValidParam("echostr");
        	if ($return) {
