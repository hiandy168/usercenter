...
class Activity_collectcard extends CActiveRecord {
    ...
    public function getActivityListPager($pid){
        $as_list = array();
        $list = null;
        $asModel = new Activity_collectcard;
    ...
    public static function activityStatus($starTime,$endTime,$id){
        ...
        $re=Activity_collectcard::model()->findByPk($id);
        if($re){
           if($re->status==0){
               $returnCode['status'] = 2;
    ...
    public static function getNum($id,$userid,$openid,$day_count){
        ...
        if($userid || $openid){
            ...
            if($openid) {
                $sql = "SELECT count(*) FROM dym_activity_collectcard_user WHERE time > $time and collectcard_id='" . $id . "' and openid='" . $openid . "'";
            }else if($userid){
                $sql = "SELECT count(*) FROM dym_activity_collectcard_user WHERE time > $time and collectcard_id='" . $id . "'  and  mid= '" . $userid . "'";
            }
