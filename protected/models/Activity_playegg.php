...
class Activity_playegg extends CActiveRecord
{
    ...
    public static function activityStatus($starTime, $endTime, $id)
    {
        ...
        $re = Activity_playegg::model()->findByPk($id);
        if ($re) {
            if ($re->status == 0) {
                $returnCode['status'] = 2;
    ...
    public static function getNum($id, $userid, $openid, $day_count)
    {
        ...
        if ($userid || $openid) {
            ...
            $sql = "SELECT count(*) FROM dym_activity_playegg_user WHERE time > $time and playegg_id='" . $id . "'  and  mid= '" . $userid . "'";
