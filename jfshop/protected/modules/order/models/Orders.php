...
class Orders extends BaseModel
{
	...
	public function relations()
	{
		...
		return array(
			'orders_items'=>array(self::BELONGS_TO, 'Orders_items','', 'on'=>'t.order_id=orders_items.order_id'),
			'goods'=>array(self::BELONGS_TO, 'Goods','', 'on'=>'orders_items.goods_id=goods.goods_id'),
		);
	}
    ...
    public function orderList($data,$pageIndex,$pageSize)
    {
        ...
        if (!empty($data)) {
            foreach ($data as $k=>$v) {
				if($k=='t.create_time'){
					$conditionArr[] = "t.createtime >$v";
				}else if($k=='t.over_time'){
					$conditionArr[] = "t.createtime < $v";
				}else{
					$conditionArr[] = "$k = '$v'";
				}
...
//		echo $sql;exit;
        ...
        $count = $this->RowCount($this->items_sql('COUNT(DISTINCT t.order_id)',$condition));

        $sql .= " GROUP BY t.order_id ORDER BY t.createtime DESC LIMIT {$start},{$pageSize}";
    ...
    {
        $sql = "SELECT {$select} FROM {{b2c_orders}} t, {{b2c_orders_items}} orders_items,{{b2c_goods}} goods ";

        if ($condition) $sql .= " WHERE t.order_id=orders_items.order_id and orders_items.goods_id=goods.goods_id and  $condition";
	...
	public function member_info($id){
		//猜你喜欢积分商城接口
		$url = "http://" . $_SERVER['HTTP_HOST'] . "/api/member/memberinfo?mid=".$id;
		$oCurl = curl_init();
		...
		curl_close($oCurl);
		$member_info = json_decode($sContent, true);
