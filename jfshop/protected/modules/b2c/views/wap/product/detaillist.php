...
<div class="div-main" id="content">
  <!--  <div class="top-title clearfix">
        <a class="lefta fl" href="javascript:history.back();void(0)">积分兑换</a>
    </div>-->
...
//        if(content_height < parseFloat($(window).height())){
        ...
        console.log(i_num);
            $.ajax({
                type: "post",
                url: siteUrl + '/b2c/wap/product/ajaxdetaillist',
...
//                                num++;
                        if (data.code === 200) {
                            var nr = "";
                            for (var i = 0; i < data.data.length; i++) {
                                var htmln = "<li><a href='/jfshop/b2c/wap/product/detail/id/"+data.data[i]['goods_id']+".html'><div class='shop-cpdiv'><img style='height: 130px;' src='<?php echo $this->_siteUrl; ?>" + "/" + data.data[i]['s_url'] + "'/><span><h3>"+data.data[i]['name'] +"</h3><h4>积分："+data.data[i]['price_jifen'] +"</h4><p>已有"+data.data[i]['con']+"人兑换</p></span></div></a></li>";
                                nr += htmln;
                            }
                            $("#li-l").append(nr);
