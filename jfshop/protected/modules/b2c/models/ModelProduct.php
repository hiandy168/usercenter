...
class ModelProduct extends B2cModel
{
    ...
    public function Item($product_id)
    {
        ...
        $row = $this->ModelQueryRow(
            "SELECT g.goods_id,g.name,g.intro,g.cat_id,g.brand_id,g.unit,g.weight,g.brief,g.image_default_id,p.product_id,p.paytype,p.price_jifen,p.price,p.cost,p.mktprice,p.spec_desc,p.store,p.freez,p.is_default FROM car_b2c_goods g LEFT JOIN car_b2c_products p ON g.goods_id = p.goods_id
        ...
        $result['row']['is_default'] = $row['is_default'];
    ...
    public function CartGoods($goods_list)
    {
        ...
        if ($product_ids) {
            ...
            foreach($product_list as $tk=>&$tp){ 
                ...
                $tp['quantity'] =   $goods_list_new[$tp['product_id']]['quantity'];
