 ...
 * Please check {@link doc/guide/database.ar the Guide} for more details
 * about this class.
 *
...
abstract class CActiveRecord extends CModel
{
	...
	/*
	 * this method is to unset $_models[$classname]
	 * for the reason when we use different table in a model
	 * before you change the table ,you need to unset pre model
	 */
	public static function dmodel($className=__CLASS__)
	{
		if(isset(self::$_models[$className]))
			unset(self::$_models[$className]);
	}
	 ...
	 *   in case of multiple scopes can be used like 'scopes'=>array('scopeName1','scopeName2').</li>
	 * </ul>
	 *
	 ...
	 * <li>'through': name of the model's relation that will be used as a bridge when getting related data. Can be set only for HAS_ONE and HAS_MANY.</li>
	 * </ul>
	 *
...
class CActiveRecordMetaData
{
	...
	public function addRelation($name,$config)
	{
		if(isset($config[0],$config[1],$config[2]))  // relation class, AR class, FK
			...
			throw new CDbException(Mod::t('mod','Active record "{class}" has an invalid configuration for relation "{relation}". It must specify the relation type, the related active record class and the foreign key.', array('{class}'=>get_class($this->_model),'{relation}'=>$name)));
	}
